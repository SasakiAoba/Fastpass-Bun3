...
describe("ログイン後の通信障害", () => {
  it("認証成功後の状態取得失敗をパスワード失敗にせず、再読込画面を出す", async () => {
    ...
    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith("/api/state", expect.anything()));
    expect(screen.queryByRole("heading", { name: "運用データを読み込めません" })).not.toBeInTheDocument();
    expect(screen.getByRole("heading", { name: "管理画面を開く" })).toBeVisible();

    resolveState(new Response("upstream failure", { status: 502, headers: { "Content-Type": "text/html", "CF-Ray": "test-ray" } }));
