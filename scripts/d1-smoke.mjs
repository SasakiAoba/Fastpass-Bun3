...
const server = spawn(wrangler, ["pages", "dev", "dist", "--binding", `AUTH_SHARED_PASSWORD=${password}`, "--persist-to", persistence, "--ip", "127.0.0.1", "--port", String(port), "--log-level", "error"], {
  cwd: root,
...
try {
  ...
  assert(health.value.schemaVersion === 3, "schema version is not 3");
  ...
  await call("/api/auth/logout", { method: "POST", body: {} });
  process.stdout.write("D1 smoke: PASS (wrong-then-correct auth, CSRF, direct DEV sale, check-in, reversal, refund, immediate use, idempotency, concurrency, purge, logout/re-login)\n");
} finally {
  server.kill("SIGTERM");
